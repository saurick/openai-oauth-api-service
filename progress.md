...
## 2026-08-09 GPT-5.6 Sol Max 档位修复

- 线上根因：本机经公网 `https://oauth-api.saurick.me/v1` 使用可回收临时 key 执行真实非流式 `/v1/responses`。旧线上版本的 `low / medium / high / xhigh` 均经 `codex_backend` 返回 200 和指定 marker，`max` 则在到达上游前被旧四档校验拦为 400 `gateway_reasoning_effort_invalid`；同时 `/v1/models` 只声明旧四档且把 Sol 默认档位误报为 `medium`。
- 边界复核：首个修复镜像放行 Max 后，本机公网探针证明 `low / medium / high / xhigh / max` 全部经 Backend 返回 200；额外探测本机 Codex 目录中的 Ultra 时，上游明确返回 400，并声明最高只接受 `max`。Ultra 是客户端编排档，不是 Backend reasoning effort；网关不将它静默映射成 Max，也不再把它声明为公网能力。
- 完成：服务端固定模型目录补齐模型级 reasoning 真源：Sol / Terra / Luna 支持 `low / medium / high / xhigh / max`，5.5 支持到 `xhigh`，Sol 默认档位为 `low`；请求解析、key / 全局覆盖、`/v1/models` 元数据、usage 记录与后台 RPC 共用该目录。非法或模型不支持的最终档位在网关本地明确返回 400，不引入 fallback。
- 完成：后台全局与 key 默认档位、usage 筛选 / 展示补齐 Max，客户端配置生成器按模型输出 opencode variants，避免生成 Backend 不接受的 Ultra 或给 5.5 生成 Max。README、API、配置与前端文档同步更新；不改 schema、migration、auth、API key 生命周期、quota、usage 历史、上游重试或代理策略。
- 本地验证：定向 biz / data / server 测试、全量 `go test -count=1 ./...`、前端 lint / css / 26 条测试 / build 均通过；`style:l1` 的 `admin-upstream-desktop`、390px `admin-upstream-mobile`、凭据桌面 / 移动和 usage 桌面共 5 个场景通过，覆盖 Max 可见、Ultra 不再暴露、暗色 / 移动布局和相邻后台页面。`bash scripts/qa/full.sh` 全部通过；govulncheck 按既有非阻断策略报告 grpc、x/text、OpenTelemetry 三项当前可达依赖告警，本轮不扩大为依赖升级。
- 正式发布：提交 `b0b7038fdaa8e536080694f9d56eb31c764654b8` 构建并部署 `oauth-api-service-server:20260809T063841Z-b0b7038f-sol-max-backend-truth`（image id `sha256:25f82403c72ec9e3f9bedfc92ca1e8a2da3555470d24b3f1af35df105ade4244`，linux/amd64）。Atlas 当前版本 `20260604123931`、19 个迁移已执行、pending 0；只重建 `app-server`，PostgreSQL 容器未变化。回滚保留发布前稳定镜像 `oauth-api-service-server:20260809T055031Z-2a7e2fa1-codexbar-usage-truth` 与 `.env.bak.20260809T062018Z-b190453e-sol-reasoning-max-ultra`，最终切换前备份为 `.env.bak.20260809T063841Z-b0b7038f-sol-max-backend-truth`。
- 线上验收：本机经公网 `/v1/responses` 对 Sol 五档逐个真实调用，`low` 1.718s（17/8/25 tokens，`sol-final-20260809T064152Z-low`）、`medium` 2.298s（18/9/27，`sol-final-20260809T064152Z-medium`）、`high` 3.850s（17/8/25，`sol-final-20260809T064152Z-high`）、`xhigh` 3.551s（18/9/27，`sol-final-20260809T064152Z-xhigh`）、`max` 1.807s（17/8/25，`sol-final-20260809T064152Z-max`）均为 200、`completed`、指定 marker 命中。usage 事实逐条确认 `codex_backend`、无 fallback、无上游错误、客户端为 Codex；`/v1/models` 返回 `low,medium,high,xhigh,max` 且 Sol 默认 `low`。Ultra 合同探针在 0.073s 返回 400 `gateway_reasoning_effort_invalid`，usage 为 1ms、0 token、无 upstream mode，证明未访问上游。临时 key `SolPost0809062733`（id 61）已删除，按备注复查残留为 0。
- 运行态验收：服务器本机与公网 `/healthz`、`/readyz` 均为 200，应用 restart count 0；启动后 WARN / ERROR / PANIC / FATAL 均为 0。容器内 `codex-cli 0.147.0` 与 npm 当前 latest `0.147.0` 一致；`/public/codex/balance` 返回 200、`status=ok`、`stale=false`，两个 limit bucket 与 reset credits 均可实时读取。
...
- 完成：`/admin-codex-balance` 的额度窗口展示已用比例、相对重置时间、可持续节奏标记和预计用尽 / 可持续到重置文案；窗口标签依 `window_duration_mins` 真值推导，不再假设 primary 固定是 5 小时。上游额度、套餐、reset credits 仍只读 `/public/codex/balance`，不由本地 usage 反推。
...
- 验证：8 条统计 / pace 单测及前端 26/26 通过，真实大额费用与 primary=10080 分钟 / secondary=null 均已纳入回归；desktop / mobile `style:l1`、人工截图与多次 `bash scripts/qa/full.sh` 全部通过。生产 Playwright 先后捕获大额格式崩溃与 primary 周窗口误标为 5 小时两个真实数据问题，最终桌面、390px 暗色页面无溢出、console 0 错误。govulncheck 仍非阻断报告三项可达依赖告警。
- Git 与部署：功能 `aad8e083c2f3`、大额格式修复 `4d8ef4a5a036` 和窗口真值修复 `2a7e2fa1c552` 已推送 `origin/main`；最终 linux/amd64 镜像 `oauth-api-service-server:20260809T055031Z-2a7e2fa1-codexbar-usage-truth` 经本地验证、远端 SHA-256 校验与 Atlas `20260604123931` / pending 0 后仅重建 `app-server`，PostgreSQL 未重启。运行时自动升级检查确认 Codex before / latest / after 均为 `0.147.0`。
- 线上验收与回滚：远端与公网 health / ready 正常，余额 `status=ok`、`stale=false`、两个 primary 均为 10080 分钟并正确显示“每周额度”，2 张可用重置券可读；管理员 30d summary / `day_model` 非空，容器 restart 0，启动后 WARN / ERROR / PANIC / FATAL 为 0。保留上一稳定镜像 `oauth-api-service-server:20260809T045013Z-3cd35ba5-quota-token-refresh`、发布包与 `.env.bak.20260809T052520Z-aad8e083-codexbar-usage`，未执行 image / builder / volume prune。
