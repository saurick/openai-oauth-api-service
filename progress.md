...
- 提交：已提交并推送 `539068e fix: 细分 Codex backend 容量错误`；push 门禁 `qa:full` 通过，包含 shellcheck、db guard、错误码同步、secrets、govulncheck、web lint/css/test/build、server 全量测试和 `make build`。
- 部署：本地构建 linux/amd64 镜像 `oauth-api-service-server:20260623T214245-539068e4-codex-overloaded`，上传到 `192.168.0.133:/data/openai-oauth-api-service/releases/20260623T214245-539068e4-codex-overloaded`。远端只执行 release 解包、宿主机 Atlas status、`docker load`、更新 `APP_IMAGE` 和 `docker compose up -d --no-deps --force-recreate app-server`，未在服务器构建；Atlas 当前版本 `20260604123931`、pending 0。远端本机和公网 `/healthz` / `/readyz` 通过，容器环境 `GIT_SHA_SHORT=539068e4`、`IMAGE_TAG=20260623T214245-539068e4-codex-overloaded`。
- 线上 smoke：管理员 `admin/adminadmin` 登录返回 `code=0`，`api.usage_list limit=3` 返回 `code=0 total=1055 items=3`，`api.gateway_upstream_get` 返回 `strategy=backend_only mode=codex_backend fallback=false`。部署后 Windows Codex `saurick-oauth` 返回 `WIN_CODEX_DEPLOY_OK_20260623`，OpenCode `oauth-api-service/gpt-5.5` 返回 `WIN_OPENCODE_DEPLOY_OK_20260623`；133 usage 落库确认 Codex `/v1/responses status=200 client_type=codex`、OpenCode `/v1/chat/completions status=200 client_type=opencode`。
- 清理：部署成功后记录远端 `/` 使用率、`docker system df` 与运行容器；删除本轮 release 的镜像 tar 包和 migration tar 包，执行 `docker image prune -a -f` 与 `docker builder prune -f`，删除未被容器使用的旧镜像 `oauth-api-service-server:20260623T141123-e6dd6e1b-key-stats-sort`，回收 353.3MB，未清理 volume。清理后根分区使用率 30%，当前 app-server 运行镜像为 `oauth-api-service-server:20260623T214245-539068e4-codex-overloaded`。
