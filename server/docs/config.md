...
`codex_backend` 模式复用同一个 app-server 进程直接请求 `https://chatgpt.com/backend-api/codex/responses`，从 `auth.json` 读取 access token，并在 access token 过期或上游返回 401 时用 refresh token 刷新后写回 `auth.json`；该模式适合高频 OpenCode 调用。默认策略是 Backend 直连，backend 请求失败时直接返回上游错误，避免把客户端本机工具错误降级为服务端 `codex exec`。确需临时救急时可在后台选择 Backend + CLI 兜底，或设置 `CODEX_UPSTREAM_FALLBACK_ENABLED=true` 作为初始环境口径，仅允许 CLI 能忠实处理的纯文本 / 图片请求 fallback；带工具调用、工具历史或文件输入的请求始终只允许走 backend。全局推理档位可在后台「上游策略」页切换，单个 API key 可继承全局、关闭覆盖或覆盖为 Fast / Medium / High / Deep；后台设置会覆盖客户端传入的 `reasoning_effort`，用于约束 Codex / OpenCode 这类会自动带默认 effort 的客户端。`codex_cli` 模式会为每次 `/v1/chat/completions` 或 `/v1/responses` 启动一次 Codex CLI，并串行执行上游请求，稳定但首包和单次延迟较高。usage 记录会同时保存配置模式、实际执行模式、fallback 状态、最终生效的 reasoning effort、Agent passthrough 标记、上下文压缩前后体积和压缩摘要，后台统计表可按上游模式筛选并展示两种模式的请求数。

Codex / OpenCode 这类 Agent 客户端默认按 `client_type=codex/opencode` 进入 Agent passthrough。该模式下网关继续负责鉴权、key 权限、quota、限流、路由、模型映射、reasoning effort 策略和 usage 记录，但不再执行网关上下文预压缩、不写入 `gateway_context_restore_state.v1`、不持久化新的 context summary，也不向 direct backend 请求补默认工程助手 prompt、visible process prompt 或 resume prompt；backend 失败时即使后台开启 CLI fallback，也会直接返回上游错误，不降级到 `codex_cli`。其他 Agent 客户端可用以下方式显式启用同样行为：

```http
X-Gateway-Agent-Passthrough: true
X-Agent-Passthrough: true
X-Raw-OpenAI-Compatible: true
```

也可以在请求体顶层或 `metadata` 中设置任一布尔开关：`passthrough`、`agent_passthrough`、`raw_openai_compatible`、`disable_auto_summarization`、`disable_context_compression`、`disable_resume_prompt_injection`、`preserve_messages`、`preserve_tool_calls`、`preserve_system_messages`、`preserve_developer_messages`。如果需要让 Codex / OpenCode 临时回到旧网关压缩路径，可设置 `X-Gateway-Agent-Passthrough: false`；该 header 会覆盖自动 client type 判定。passthrough 请求若超过真实上游上下文窗口，会保留上游的 `context_length_exceeded` / `response.incomplete` 等错误分类并写入 usage diagnostic，而不是由网关提前改写消息。

非 passthrough 请求的网关上下文压缩只保存工程摘要，不保存完整 prompt 或模型输出正文。压缩会尽量保留系统 / developer 指令、最近若干轮未压缩消息和最近完整工具闭环；较早历史会收口为 `gateway_context_restore_state.v1` JSON 状态包，而不是自由散文。状态包显式区分 `current_user_goal`、`latest_user_instruction`、`latest_uncompressed_user_instruction`、`pinned_raw_user_directives`、`current_effective_constraints`、`must_not_do`、`current_task_phase`、`next_action`、`restore_audit`、`historical_context_only`、`obsolete_or_superseded_goals` 和 `obsolete_or_superseded_constraints`，避免恢复后从历史路径、日志或旧目标碎片里自行生成新目标。`latest_user_instruction` 只从 user-eligible 片段提取，assistant / tool 输出、恢复规则说明和旧状态包字段不会作为当前用户指令；若最新未压缩用户消息明确“继续执行 / 允许工具调用”，旧的 `stopped`、`must_not_do` 和 `requires_user_confirmation_before` 会移入 obsolete，而不是继续阻塞当前任务。单个超长 `input`、`content` 或 `function_call_output` 被裁剪时，不只保留固定短尾部，还会优先提取“当前请求 / 最新会话进度 / 当前状态 / 下一步 / 验证 / 部署 / 阻塞 / 风险 / 停止 / 暂停 / 不要”等附近文本；多消息数组压缩时也会插入显式恢复锚点，并从被裁掉片段尾部反向提取最近用户请求。若数组里保留下来的最近消息本身仍是超大 user / tool 内容，网关会继续对该 item 做二次大内容压缩；若客户端把最新大文本放在非标准位置，网关还会对 `input/content/text/arguments/output/instructions` 的超大字符串做通用二次压缩，但跳过 tools schema 和 metadata。这样可以避免显式 resume 后最新请求原样撑爆压缩后请求，也避免 Codex 压缩恢复后被系统规则或末尾噪声带偏。网关能把停止、只读、需要确认等指令写入状态包和恢复提示，但真正拒绝客户端本地工具调用仍取决于客户端 / runtime 是否执行这些字段；本服务不会代理本机 shell、文件或 SSH 工具。压缩阈值按“模型级配置 > 环境变量运维覆盖 > 内置模型推荐值 > 旧默认兜底”的顺序生效；模型级配置在后台模型管理页保存后立即影响后续请求，不需要重启服务。内置推荐按 Codex 使用体验控制在 `400K` 上下文窗口内，默认 `260K` 开始压缩、`380K` 硬拦截，字节阈值默认 `1M` / `1.9M`；如果确实要使用 API 模型更大的长上下文窗口，应在后台按模型显式覆盖，并接受更高 token 消耗和 long-context 价格档。后台阈值输入支持整数、`K` 和 `M` 单位，例如 `260K` 表示 260000，`0.38M` 表示 380000；保留条数只接受普通整数。若压缩后仍超过硬阈值，网关会在转发前返回 `context_length_exceeded`，避免 Codex CLI 对同一超长请求连续重试。
...
- usage 费用估算优先使用数据库模型价格表覆盖值；未配置覆盖值时，回落到服务端内置 Codex 客户端可用模型中已定价模型的价格表。当前默认模型为 ChatGPT Codex backend 可用的 `gpt-5.6-sol`，候选集合另含已真实验证的 `gpt-5.6-terra`、`gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、`gpt-5.3-codex`、`gpt-5.3-codex-spark`、`gpt-5.2`；OpenAI API 的无后缀 `gpt-5.6` Sol alias 不作为本项目 Codex backend 模型 ID。官方已发布的 `gpt-5.6-luna` 在当前 ChatGPT Codex backend / 账号仍返回 404，暂不进入可用目录和费用估算，待真实 rollout 可用后再接入。`gpt-5.3-codex-spark` 为 research preview，价格未定，不进入费用估算单价表。GPT-5.6 Sol / Terra 按官方短上下文 Standard token 单价估算；超过 272K 输入的长上下文加价、cache write、Batch、Flex、Priority 和区域处理加价暂不纳入当前估算口径。
