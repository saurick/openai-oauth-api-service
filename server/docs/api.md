...
返回当前代码内置的 Codex 客户端可用模型中已定价模型的价格表，用于前端费用展示和费用估算兜底；当前已定价模型为 `gpt-5.6`、`gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、`gpt-5.3-codex`、`gpt-5.2`：
...
说明：价格单位为 USD / 1M tokens。`gpt-5.6` 是 GPT-5.6 Sol 的官方别名；GPT-5.6 三档当前记录官方短上下文 Standard 输入、缓存输入和输出单价。`gpt-5.3-codex-spark` 保留在 Codex 客户端可用模型候选中，但仍是 research preview，价格未定，不进入费用估算单价表；超过 272K 输入的长上下文加价、cache write、Batch、Flex、Priority 和区域处理加价需要新增口径字段后再接入。
