...
func gatewayUsageFilterFromParams(pm map[string]any) biz.GatewayUsageFilter {
	...
	return biz.GatewayUsageFilter{
		...
		ClientIP:          getString(pm, "client_ip"),
		...
		ErrorType:         getString(pm, "error_type"),
		ExcludeErrorType:  getString(pm, "exclude_error_type"),
		StatusCode:        getInt(pm, "status_code", 0),
		StartTime:         start,
		EndTime:           end,
...
func mapGatewaySummary(item *biz.GatewayUsageSummary) map[string]any {
	...
	data := map[string]any{
		...
		"client_canceled_requests": item.ClientCanceled,
		"total_tokens":             item.TotalTokens,
		"input_tokens":             item.InputTokens,
		"output_tokens":            item.OutputTokens,
		"cached_tokens":            item.CachedTokens,
