...
用这个 skill 在实现 `openai-oauth-api-service` 功能前收敛 domain ownership、source of truth、API/RBAC、frontend/backend responsibility，以及 OAuth、API key、usage、upstream 和 persisted-config 边界。
...
- OAuth 状态、API key/quota、usage、upstream/fallback、persisted config 的残值/缺值、幂等、事务与权限必须可测试、可解释、可回滚；不能只让当前 happy path 通过。
...
4. 覆盖 stale/missing/readback paths：defaults、edits、upstream/config switch or clear、list/detail/usage/query、restart 后持久化和 fallback。
5. UI 不补造 OAuth、quota、usage 或 upstream facts；环境/部署差异不污染服务核心合同。
