...
- 测试通过不能替代 OAuth/API key/quota/usage/upstream 真源、可维护性、可回滚性和文档同步判断。
...
| Live Gateway / Context Compaction | 真实上游、长上下文压缩与 session 连续性 | `python3 scripts/qa/live-context-compaction.py ...`；显式 target URL/key/session，消耗真实额度，不进入 fast/full/strict |
| Deploy Preflight | Compose、低配发布、运行时配置 | `bash scripts/deploy/production-preflight.sh --env-file server/deploy/compose/prod/.env --runtime`，再做 health/ready |
...
- 真实 context-compaction 回归必须显式授权目标 URL、key 和本轮 session，输出脱敏；未运行时单列为 live-upstream 盲区，不能让 fast/full/strict 暗示已覆盖。
