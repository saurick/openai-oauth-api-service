...
Codex 额度页同时展示两类数据，但不合并口径：上游账户额度、重置时间与 reset credits 只来自 `/public/codex/balance`；Today / 30d Token、费用估算、主模型和日柱图只来自 `gateway_usage_logs` 的管理端汇总。节奏提示根据当前额度窗口的已用比例、窗口时长和重置时间做线性估算，窗口时间进度不足 3% 时不预测；当前没有账户级额度历史真源，因此不宣称具备 CodexBar 的历史个性化预测。费用是本地模型价格估算，不是 Codex 订阅账单，也不代表未经过本服务的账户调用。
