...
2. 已迁入 Go 后端主路径：管理员登录、下游 key、模型缓存、usage log、OpenAI 兼容转发和统一上游代理。
3. 已补管理后台主入口：key 创建/启停、模型启停、usage 汇总和最近请求。
4. 管理员授权登录入口已从当前主路径移除，只保留后台账号登录。
5. 账号目录和普通用户门户不作为当前前端主路径；后端历史 `users` 相关能力和 OAuth 绑定字段仅作为兼容实现保留，后续如需彻底移除需单独评估数据迁移与 key 归属字段。
