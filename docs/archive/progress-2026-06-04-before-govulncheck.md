...
- 部署：已提交并推送 `1c557bb feat: 完善后台用量统计` 和 `926470f docs: 记录后台用量部署结果`；最终本地构建 linux/amd64 镜像 `oauth-api-service-server:20260604T062400-926470fb-usage-client-type` 并上传到 `192.168.0.133:/data/openai-oauth-api-service/releases/20260604T062400-926470fb-usage-client-type`。远端只执行 `docker load`、宿主机 Atlas migration/status、更新 `APP_IMAGE` 和 `docker compose up -d --no-deps --force-recreate app-server`，未在服务器构建。Atlas 从 `20260531143157` 应用到 `20260604051355`，待执行 migration 为 0；本机与公网 `/healthz` / `/readyz` 通过，管理员 `admin/adminadmin` 登录、`api.summary`、`api.usage_list client_type=opencode` 和临时 key `/v1/models` smoke 通过，临时 key 已删除。
- 清理：部署成功后记录 `df -h /`、`docker system df` 与运行容器；执行 `docker image prune -a -f` 和 `docker builder prune -f`，删除旧镜像 `oauth-api-service-server:20260531T150828-619b8785-fast-effort` 和过渡镜像 `oauth-api-service-server:20260604T055322-1c557bb6-usage-client-type`，未清理 volume。根分区使用率保持 20%，当前 app-server 运行镜像为 `oauth-api-service-server:20260604T062400-926470fb-usage-client-type`。
...
- 再次补充完成：`/admin-client-config` 和 `/client-config` 的 API Key 输入框改为默认空值与示例 placeholder，预览区保留 `<API_KEY>` 占位；复制或下载前若未填写 API Key，会显示后台统一样式弹窗提示先填写，不再使用浏览器原生提示；两个页面标题、说明和教程文案统一为“客户端配置生成器”，移除“固化进仓库”这类面向维护者的描述。
- 再次补充验证：`cd web && pnpm exec eslint --ext .js --ext .jsx src/pages/AdminClientConfig/ClientConfigBuilder.jsx src/pages/AdminClientConfig/index.jsx src/pages/PublicClientConfig/index.jsx src/common/components/modal/AlertDialog.jsx src/common/components/modal/AppAlertProvider.jsx src/common/components/layout/AdminFrame.jsx scripts/styleL1.mjs`、`cd web && node --check scripts/styleL1.mjs`、`cd web && pnpm test`、`cd web && pnpm build`、`cd web && STYLE_L1_PORT=4385 NODE_USE_ENV_PROXY=0 STYLE_L1_SCENARIOS=admin-client-config-desktop,admin-client-config-mobile,public-client-config-desktop,public-client-config-mobile pnpm style:l1` 均通过；本地 Playwright 复测公开页空 Key 弹窗、填写后 opencode 预览与剪贴板内容、后台页下载前空 Key 弹窗、后台公开页入口和无横向溢出均通过。
- 再次提交推送：已提交并推送 `3cc079a`（`feat: 优化客户端配置 key 填写校验`）到 `origin/main`；pre-push 全量门禁通过，`govulncheck` 仍仅提示 Go 1.25.9 / `x/net` 既有漏洞警告。
- 再次部署：本地构建 `linux/amd64` 镜像 `oauth-api-service-server:20260526T225649-3cc079ac-client-config-key`，上传 release 到 `/data/openai-oauth-api-service/releases/20260526T225649-3cc079ac-client-config-key`；远端 `sha256sum` 校验通过，`docker load` 后通过宿主机 `/usr/local/bin/atlas` 确认 migration 已是最新；更新 compose `.env` 前备份为 `.env.bak.20260526T225909-client-config-key`，随后 `--force-recreate` 重建 `app-server`。
- 再次线上验证：远端容器运行镜像已切到 `oauth-api-service-server:20260526T225649-3cc079ac-client-config-key`，`service.version=3cc079ac3dcf63899c7cf451f2876a5e1cfeb125`；本机和公网 `/healthz` 返回 `ok`、`/readyz` 返回 `ready`；公网 `/client-config` 与 `/admin-client-config` 均返回 `HTTP 200` 并加载 `assets/index.BG-0MA0M.js`，产物内包含「请先填写 API Key」「粘贴你的 API Key」「复制或下载前请填写 API Key」和「客户端配置生成器」，且不包含「固化进仓库」；后台 `admin_login` 与 `summary` RPC 均返回 `code=0`。
- 再次发布清理：远端删除本次上传的压缩镜像包与校验文件，执行 `docker image prune -a -f` 和 `docker builder prune -f`，未执行 volume prune；根分区使用率从 56% 降到 53%，Docker 镜像占用从 5.14GB 降到 4.427GB，当前业务容器和数据库容器保持运行。
- 阻塞/风险：本轮未处理 pre-push 中 `govulncheck` 报出的既有依赖 / Go 版本漏洞警告；部署过程中第一次 `.env` 更新脚本因少传变量在写入前失败，只额外留下未使用备份 `.env.bak.20260526T225854-client-config-key`，第二次脚本已成功更新并重建容器。
