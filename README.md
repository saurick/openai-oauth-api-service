...
| [`.agents/skills/`](.agents/skills/README.md) | Codex repo skills | 项目专项代码审查、文档、服务边界、页面、测试和统一 operations；通用提示词与 Git 收口使用全局 skills |
...
API 转发统一使用服务器 Codex 登录态，默认只走 direct Codex backend，backend 失败时直接返回上游错误，避免把客户端本机工具错误改成服务端 `codex exec`。管理后台「上游策略」页可以运行时切换 Backend 直连、Backend + CLI 兜底和强制 CLI 三种策略，并可设置全局推理档位开关；全局开关默认关闭，开启后会覆盖客户端传入的 `reasoning_effort`，单个 key 可单独覆盖或关闭。确需临时救急时才选择 CLI 兜底，且带工具调用、工具历史或文件输入的请求仍不会 fallback 到 CLI。`CODEX_UPSTREAM_MODE` 只作为未保存后台设置时的启动默认值：
...
- 管理登录：`/admin-login`
- API 运营控制台：`/admin-api`
- Codex 余额 / 限额公开查询：`GET /public/codex/balance`
...
`/public/codex/balance` 不要求管理员登录。服务端会先复用 direct backend 的统一凭据入口检查服务器 Codex access token，临近到期或已经到期时用 refresh token 刷新并安全写回同一 `auth.json`，再启动 Codex app-server 读取 `account/rateLimits/read`，同时只读获取 rate limit reset credits。接口只返回窗口用量百分比、剩余额度百分比、重置时间、plan type、credits 余额、重置券数量以及重置券的类型 / 状态 / 获得和过期时间；不会返回账号邮箱、access token、refresh token、上游内部 credit id 或请求正文。refresh token 无效时接口仍会诚实失败，需要维护者重新登录服务器 Codex；生产环境如不希望任意人看到余额，应在 Nginx / Cloudflare 层增加 IP allowlist 或独立查询 token。
...
兼容请求可通过 `reasoning_effort` 传递推理强度。`gpt-5.6-sol`、`gpt-5.6-terra` 与 `gpt-5.6-luna` 支持 `low`、`medium`、`high`、`xhigh`、`max`，`gpt-5.5` 支持到 `xhigh`；模型不支持的档位返回 400，不静默降级。本机 Codex 目录里的 `ultra` 是客户端编排档，不是 Codex Backend 接受的 reasoning effort，本服务不会把它伪装或映射为 `max`。服务端按“key 覆盖档位 > 全局覆盖档位 > 客户端请求档位”的顺序决定最终值，key 级 `none` 表示即使全局开启也不覆盖该 key。服务端会把最终生效的值作为请求级 usage 快照记录，并分别转为 direct backend 的 `reasoning.effort` 或 Codex CLI 的 `model_reasoning_effort`。direct backend 模式支持 OpenAI-compatible 的 `tools` / `tool_choice`、assistant `tool_calls` 历史和 tool result 回传，可供 Codex / OpenCode 这类客户端继续在本机执行 shell、文件和截图等工具；这类 Agent 客户端默认走 passthrough：网关不做上下文预压缩、不注入恢复进度提示，也不补默认工程助手 prompt，由客户端自己的 compact / resume 机制维护任务状态。其他客户端可通过 `X-Gateway-Agent-Passthrough: true` 或请求体 / `metadata` 中的 `passthrough`、`raw_openai_compatible`、`disable_context_compression` 等布尔开关显式启用；确需临时回到旧网关压缩路径时，Agent 客户端可设置 `X-Gateway-Agent-Passthrough: false`。Codex CLI fallback 默认关闭，打开后也只返回纯文本，不代理工具调用；带工具调用、工具历史、文件输入或 Agent passthrough 的请求在 backend 失败时会直接返回上游错误，不会降级到 CLI。图片输入支持 OpenAI-compatible 的 data URL 形式 `image_url` / `input_image`；PDF 输入支持 `input_file` / `file` 的 `application/pdf` data URL 或带 `mimeType=application/pdf` 的 base64 文件数据。图片在 CLI 模式会转为 `codex exec --image` 附件，PDF 仅支持 direct backend 模式；图片 / PDF 单个附件最大 16 MiB、单次最多 4 个，网关总请求体上限 90 MiB；文本类附件由客户端读取成文本后按普通 `text` 输入转发。
