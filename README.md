...
API 转发统一使用服务器 Codex 登录态，默认只走 direct Codex backend，backend 失败时直接返回上游错误，避免把客户端本机工具错误改成服务端 `codex exec`。管理后台「上游策略」页可以运行时切换 Backend 直连、Backend + CLI 兜底和强制 CLI 三种策略；确需临时救急时才选择 CLI 兜底，且带工具调用、工具历史或文件输入的请求仍不会 fallback 到 CLI。`CODEX_UPSTREAM_MODE` 只作为未保存后台设置时的启动默认值：
