...
export function gatewayErrorTypeStatusHint(code) {
  return gatewayErrorTypeInfo(code)?.statusHint || ''
}

export function gatewayErrorTypeFilterLabel(code, fallbackLabel = '') {
  const label =
    fallbackLabel || gatewayErrorTypeLabel(code) || String(code || '')
  const statusHint = gatewayErrorTypeStatusHint(code)
  return statusHint ? `${statusHint} · ${label}` : label
}
